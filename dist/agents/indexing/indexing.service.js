...
let IndexingService = IndexingService_1 = class IndexingService {
    documentService;
    config;
    logger = new common_1.Logger(IndexingService_1.name);
    constructor(documentService, config) {
        this.documentService = documentService;
        this.config = config;
    }
    async indexDocument(data, onProgress) {
        ...
        this.logger.debug(`[${correlationId}] ${filteredTokens.length} tokens après filtrage stop-words`);
        onProgress(60);
        const cleanedText = filteredTokens.join(' ');
        const updatedMetadata = {
            ...metadata,
            _indexedText: cleanedText.slice(0, 10000),
            _language: language,
            _indexedTokens: filteredTokens.length,
        };
        onProgress(80);
        await this.documentService.markIndexed(documentId);
        this.logger.log(`[${correlationId}] Document ${documentId} indexé : ${filteredTokens.length} tokens significatifs`);
        onProgress(100);
    }
};
