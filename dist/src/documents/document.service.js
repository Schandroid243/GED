"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
var DocumentService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.DocumentService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const document_entity_1 = require("./entities/document.entity");
const document_status_enum_1 = require("./enums/document-status.enum");
let DocumentService = DocumentService_1 = class DocumentService {
    documentRepo;
    logger = new common_1.Logger(DocumentService_1.name);
    constructor(documentRepo) {
        this.documentRepo = documentRepo;
    }
    async updateStatus(documentId, status) {
        await this.documentRepo.update(documentId, { status });
        this.logger.debug(`Document ${documentId} → statut ${status}`);
    }
    async updateClassification(documentId, documentType, confidence) {
        await this.documentRepo.update(documentId, {
            documentType,
            classificationConfidence: confidence,
            status: document_status_enum_1.DocumentStatus.CLASSIFIED,
        });
        this.logger.debug(`Document ${documentId} classifié : ${documentType} (${confidence})`);
    }
    async markIndexed(documentId) {
        await this.documentRepo.update(documentId, {
            indexed: true,
            status: document_status_enum_1.DocumentStatus.INDEXED,
        });
        this.logger.debug(`Document ${documentId} indexé`);
    }
    async markArchived(documentId) {
        await this.documentRepo.update(documentId, {
            status: document_status_enum_1.DocumentStatus.ARCHIVED,
        });
        this.logger.debug(`Document ${documentId} archivé`);
    }
    async markError(documentId, reason) {
        await this.documentRepo.update(documentId, {
            status: document_status_enum_1.DocumentStatus.DOCUMENT_ERROR,
        });
        this.logger.error(`Document ${documentId} en erreur : ${reason}`);
    }
    async findById(documentId) {
        return this.documentRepo.findOne({ where: { id: documentId } });
    }
    async findByTenant(tenantId, options) {
        const where = { tenantId };
        if (options?.status) {
            where.status = options.status;
        }
        return this.documentRepo.findAndCount({
            where,
            take: options?.limit ?? 100,
            skip: options?.offset ?? 0,
            order: { updatedAt: 'DESC' },
        });
    }
};
exports.DocumentService = DocumentService;
exports.DocumentService = DocumentService = DocumentService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(document_entity_1.Document)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], DocumentService);
//# sourceMappingURL=document.service.js.map